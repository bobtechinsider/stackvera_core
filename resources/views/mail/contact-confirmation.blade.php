@php
    ...
    $calendarUrl = 'https://calendar.app.google/vGVv6Sh6EPv8do3M8';
@endphp
<!DOCTYPE html>
...
<head>
    ...
    <title>{{ __('contact.confirmation.subject') }}</title>
                ...
                <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="width:600px; max-width:600px; background-color:#ffffff; border-radius:16px; overflow:hidden; box-shadow:0 1px 24px -12px rgba(17,33,56,0.35);">
                    ...
                    <tr>
                        <td style="padding:8px 40px 0 40px;">
                            <h1 style="margin:0; font-family:{{ $headingFont }}; font-size:22px; font-weight:700; color:{{ $brandNavy }};">{{ __('contact.confirmation.heading') }}</h1>
                            <p style="margin:12px 0 0 0; font-size:14px; line-height:22px; color:rgba(17,33,56,0.7);">
                                {{ __('contact.confirmation.intro', ['name' => $enquiry->name]) }}
                            </p>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td style="padding:20px 40px 8px 40px;">
                            <p style="margin:0 0 8px 0; font-size:11px; font-weight:600; letter-spacing:0.08em; text-transform:uppercase; color:{{ $brandPurple }};">{{ __('contact.confirmation.message_label') }}</p>
                    ...
                    <tr>
                        <td align="center" style="padding:24px 40px 8px 40px;">
                            <a href="{{ $calendarUrl }}" style="display:inline-block; padding:14px 32px; background-color:{{ $brandPurple }}; color:#ffffff; font-family:{{ $headingFont }}; font-size:15px; font-weight:700; text-decoration:none; border-radius:999px;">{{ __('contact.confirmation.cta') }}</a>
                        </td>
                    </tr>

                    <tr>
                        <td style="padding:16px 40px 36px 40px;">
                            <p style="margin:0; font-size:14px; line-height:22px; color:rgba(17,33,56,0.7);">
                                {{ __('contact.confirmation.closing') }}<br>
                                <strong style="color:{{ $brandNavy }};">{{ __('contact.confirmation.team') }}</strong>
                            </p>
                        </td>
                    </tr>
                </table>
