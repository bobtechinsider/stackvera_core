...
new class extends Component {
    ...
    public function submit(): void
    {
        ...
        if (RateLimiter::tooManyAttempts($rateLimiterKey, 5)) {
            $this->addError('email', __('contact.rate_limited'));
        ...
        Notification::route('mail', $enquiry->email)
            ->notify((new ContactEnquiryConfirmation($enquiry))->locale(app()->getLocale()));
...
<div>
    ...
    <form wire:submit="submit" class="space-y-5">
        ...
        <div class="grid gap-5 sm:grid-cols-2">
            <div>
                <label for="name" class="mb-1.5 block text-sm font-semibold text-[#112138]">{{ __('contact.form.name') }}</label>
            ...
            <div>
                <label for="company" class="mb-1.5 block text-sm font-semibold text-[#112138]">{{ __('contact.form.company') }}</label>
        ...
        <div>
            <label for="email" class="mb-1.5 block text-sm font-semibold text-[#112138]">{{ __('contact.form.email') }}</label>
        ...
        <div>
            <label for="message" class="mb-1.5 block text-sm font-semibold text-[#112138]">{{ __('contact.form.message') }}</label>
        ...
        <button type="submit" wire:loading.attr="disabled" wire:target="submit"
            ...
            <span wire:loading.remove wire:target="submit">{{ __('contact.form.send') }}</span>
        ...
        <p class="text-xs text-[#112138]/45">{{ __('contact.form.consent') }}</p>
    </form>
</div>
